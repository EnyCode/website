...
And, if you want to *not* disconnect after ~30 seconds, you need to respond to a [Keep Alive Packet](https://wiki.vg/Protocol#Keep_Alive) with an equal payload, just a different packet id. 

## Minecraft sends a LOT of packets...
With no entities and a vanilla Minecraft server, `crafti` receives ~450 packets a **second**. With another player spectating, we receive around 500-600. However, right now, with our *extremely* basic functionality, we only care about 2 types of packets:
...
*Feel free to subscribe to receive weekly email updates to Crafti devlogs*
*I promise you'll only get 1 weekly email for a devlog, nothing else.*
